...
if args.output=='':
    args.output='./res/mbpa-result-21.txt'
...
if __name__ == '__main__':
    ...
    # 一直就用这一个模型
        ...
        # args.trainmode = '21'
        # 否则这样就行了
        model_test = MbPAplusplus(args,model_state=torch.load('mbpa/ckpt/20.pth'))
        print('load '+'mbpa/ckpt/20.pth  successed!')
        buffer = {}
        with open('mbpa/ckpt/20.pkl', 'rb') as f:
            buffer = pickle.load(f)
        memory =ReplayMemory(buffer=buffer)
        print('load '+'mbpa/ckpt/20.pkl'+' successed!')
