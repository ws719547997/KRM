...
class Appr(object):
    ...
    def train_epoch(self, t, data, iter_bar):
        ...
        for step, batch in enumerate(iter_bar):
            ...
            layer_old=None
            if t > 0:
                _, layer_old = self.model_old.forward(input_ids, segment_ids, input_mask)

            # Forward
            outputs, layer = self.model.forward(input_ids, segment_ids, input_mask)
            output = outputs[t]
            loss = self.ce(layer_old, layer, output, targets)
    ...
    def ce(self, layer_old, layer, output, targets):
