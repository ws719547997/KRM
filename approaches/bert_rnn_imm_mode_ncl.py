...
class Appr(object):
    def __init__(self, model,taskcla, nepochs=100, sbatch=64, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=3, clipgrad=10000,
        ...
        print('CONTEXTUAL + IMM_MODE + RNN NCL')
    ...
    def train(self, t, train, valid, args):
        ...
        for e in range(self.nepochs):
            ...
            if t == 0:
                iter_bar2 = tqdm(train, desc='Fisher Information Iter (loss=X.XXX)')
                self.fisher = utils.fisher_matrix_diag(t, train,self.model,iter_bar2,self.device, self.ce)
            else:
                iter_bar2 = tqdm(train, desc='Fisher Information Iter (loss=X.XXX)')
                fisher_new = utils.fisher_matrix_diag(t, train,self.model,iter_bar2,self.device, self.ce)
