...
class Appr(ApprBase):
    ...
    def train(self,t,train,valid,args,num_train_steps=None,train_data=None,valid_data=None):
    ...
    def train_epoch(self,t,data,iter_bar):
        ...
        for step, batch in enumerate(iter_bar):
            ...
            self.control_1_s = (self.control_1_s - torch.min(self.control_1_s)) / (
                    torch.max(self.control_1_s) - torch.min(self.control_1_s))
            control_1_sorted, control_1_indices = torch.sort(self.control_1_s)

            control_1_mask = torch.zeros_like(self.control_1_s).cuda()
            control_1_mask[:int(self.control_1_s.size(-1) * s)] = 1 - control_1_sorted[
                                                                      :int(self.control_1_s.size(-1) * s)]

            self.control_2_s += torch.abs(control_2.sum(0))
            self.control_2_s = (self.control_2_s - torch.min(self.control_2_s)) / (
                    torch.max(self.control_2_s) - torch.min(self.control_2_s))
            control_2_sorted, control_2_indices = torch.sort(self.control_2_s)

            control_2_mask = torch.zeros_like(self.control_2_s).cuda()
            control_2_mask[:int(self.control_2_s.size(-1) * s)] = 1 - control_2_sorted[
                                                                      :int(self.control_2_s.size(-1) * s)]

            self.control_3_s += torch.abs(control_3.sum(0))
            self.control_3_s = (self.control_3_s - torch.min(self.control_3_s)) / (
                    torch.max(self.control_3_s) - torch.min(self.control_3_s))
            control_3_sorted, control_3_indices = torch.sort(self.control_3_s)

            control_3_mask = torch.zeros_like(self.control_3_s).cuda()
            control_3_mask[:int(self.control_3_s.size(-1) * s)] = 1 - control_3_sorted[
                                                                      :int(self.control_3_s.size(-1) * s)]
